...
function parseSlots(value: string | undefined) {
    return [...new Set((value ?? '').split(',').map((slot) => slot.trim()).filter(Boolean))]
...
export default class BookingsController {
    ...
    async details({ view, request, response }: HttpContext) {
        ...
        if (!court || !bookingDate || !slotArr.length || slotArr.length > MAX_SLOTS || !slotArr.every(isValidSlot) || !isConsecutive(slotArr)) {
    ...
    async store({ request, response, session }: HttpContext) {
        ...
        const bookingSlots = parseSlots(
            Array.from(
                { length: (slotToMinutes(bookingData.time_end) - slotToMinutes(bookingData.time_start)) / 60},
                (_, index) => {
                    const hour = Number(bookingData.time_start.substring(0, 2)) + index
        ...
        if (hasBookedSlot) {
            ...
            return response.redirect(`/booking?court=${bookingData.court_id}&date=${bookingData.booking_date}`)
        }
    ...
    async paymentInit({ request, response, session}: HttpContext) {
        ...
        if (!court || !slotArr.length || slotArr.length > MAX_SLOTS || !slotArr.every(isValidSlot) || !isConsecutive(slotArr)) {
            return response.redirect('/booking')
        }
